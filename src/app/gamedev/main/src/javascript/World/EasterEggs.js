        ...
        this.eggs.add = (_options) =>
        {
            ...
            egg.collisionCallback = (_event) =>
            {
                ...
                if(_event.body === this.physics.car.chassis.body && !egg.found)
                {
                    ...
                    {
                        if(window.confirm(`
You found a hidden GameDev egg!
Use this secret code in our wing announcements:
${code}

Would you like to open Geekhaven now?
                        `))
                        {
                            window.open('https://geekhaven.iiita.ac.in/', '_blank')
                        }
